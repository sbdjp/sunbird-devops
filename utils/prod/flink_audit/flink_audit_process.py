...
client_secrete_json = sys.argv[1]

uniq_tele_bucket_path = config['Bucket_Path']['unique_bucket'] + "{}-*".format(date)
denorm_tele_bucket_path = config['Bucket_Path']['denorm_bucket'] + "{}-*".format(date)
uniq_summary = config['Bucket_Path']['unique_summary'] + "{}-*".format(date)
denorm_summary = config['Bucket_Path']['denorm_summary'] + "{}-*".format(date)
raw_tele_bucket_path = config['Bucket_Path']['raw_bucket'] + "{}-*".format(date)
ingestion_tele_bucket = config['Bucket_Path']['ingestion_bucket'] + "{}-*".format(date)
...
def adhoc_analysis():
    ...
    df_ingestion = df_ingestion.withColumn('Date', F.lit(date))
    ...
    df_raw = df_raw.withColumn('Date', F.lit(date));
    ...
    df_uniq_raw = df_uniq_raw.withColumn('Date', F.lit(date));
    ...
    df_uniq2_raw = df_uniq2_raw.withColumn('Date', F.lit(date));

    df_denorm = spark.read.json(path=denorm_tele_bucket_path, schema=raw_schema).select("mid").distinct();
    df_denorm = df_denorm.withColumn('Date', F.lit(date));
    ...
    df_raw_summary = df_raw_summary.withColumn('Date', F.lit(date));

    df_denorm_summary = spark.read.json(path=denorm_summary).select("mid").distinct();
    df_denorm_summary = df_denorm_summary.withColumn('Date', F.lit(date));
    ...
    frame.to_csv('/mount/data/analytics/scripts/flink_audit/flink_audit.csv', index=0)
    ...
    base_dir = '/mount/data/analytics/scripts/flink_audit/flink_audit.csv'
