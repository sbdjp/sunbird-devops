#!/bin/bash
# Title: Configuring error log sync settings
# Description: Technical document on configuring error log sync settings
...
# Check the status of errorLogSyncSettings

curl -s -X GET \
  $sunbirdBaseURL/api/data/v1/system/settings/get/errorLogSyncSettings \
  -H "Authorization: Bearer $Bearer" \
  -H 'Content-Type: application/json'
